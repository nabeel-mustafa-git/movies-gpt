const checkValidateData = (name, email, password) => {
  ...
  if (!nameCheck) return ["Enter valid name:", "First letter should be capital. Must be greater than 2 letters."];
  if (!emailCheck) return ["Enter Valid Email."];
  if (!passwordCheck)
    return ["Enter Valid Password:", "Must be greater than 8 letters. At least one capital letter. At least one small letter. At least one number."];
