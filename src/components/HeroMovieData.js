        ...
        <div>
          ...
          <div className="flex gap-2 py-2 flex-wrap max-sm:text-sm">
            {data?.genres.map((item, i) => (
              <p key={i} className="bg-black bg-opacity-50 px-4 py-1 rounded-full border border-gray-700 text-gray-200">
                {item?.name}
              </p>
            ))}
          </div>
          <p className="font-light max-w-[500px] max-sm:text-sm text-gray-200">{data.overview}</p>
          <div className="flex gap-2 py-5">
            <button className="py-1 px-5 bgrd-red rounded-sm flex items-center gap-1 hover:bg-red-800 duration-150">
            ...
            <button className="py-1 px-5 bg-gray-800 border border-gray-400 rounded-sm hover:bg-opacity-50 duration-150">Read more</button>
          </div>
        </div>
      )}
    </div>
